...
namespace DatabaseUtilities
{
    public abstract class DatabaseWorker : IDisposable
    {
        ...
        protected ProcessStatus BulkInsert<T>(List<T> data, string destinationTable, bool isTransactional = false, int commandTimeOut = 10800) where T : class
        {
            ProcessStatus result = new ProcessStatus();
            try
            {
                ...
                {
                    result.IsSuccess = true;
                    SqlBulkCopy bulkCopy = new SqlBulkCopy(_connection, SqlBulkCopyOptions.KeepIdentity & SqlBulkCopyOptions.KeepNulls, _transaction);
        ...
        public ProcessStatus SaveChanges()
        {
            ProcessStatus result = new ProcessStatus();
            try
            {
